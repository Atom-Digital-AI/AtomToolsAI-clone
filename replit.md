...
- **Universal Admin Error Reporting**: Created `useAdminToast` hook that automatically adds "Send to AI" button to ALL error toasts for admin users. This is a drop-in replacement for the standard `useToast` hook. When admin users see error toasts, they can instantly send the error details (including URL, user agent, timestamp, and error context) to the AI agent for debugging. The hook wraps the standard toast functionality and intercepts destructive toasts to add the admin reporting button. Available in `client/src/hooks/use-admin-toast.ts` (October 17, 2025)
- **Content Writer v2 Style Matching Split Test**: Implemented A/B testing for brand style matching methods in Content Writer v2. Added `styleMatchingMethod` field to content_writer_sessions schema with values 'continuous' (default) or 'end-rewrite'. **Continuous Method** (existing behavior): Brand context is retrieved and injected at every stage (concepts, subtopics, article generation) using top 5 chunks at 0.7+ similarity. **End-Rewrite Method** (new A/B test): Brand context is skipped during all generation stages; content is created naturally without brand influence, then a final rewrite step retrieves ALL brand context (20 chunks at 0.6+ similarity), analyzes the brand's writing style, and rewrites the entire article to match the analyzed brand voice. All 5 content generation endpoints (concepts, regenerate concepts, subtopics, more subtopics, article generation) now check styleMatchingMethod and conditionally skip brand context when set to 'end-rewrite'. UI includes radio buttons for method selection with clear explanations of each approach (October 17, 2025)
- **Error Tracking System with Status Management**: Implemented comprehensive error logging system that automatically captures all application errors with status tracking. Added `status` field to error_logs table with values 'to_do', 'investigated', 'fixed'. Global error handler middleware automatically logs all unhandled errors to database with full context (request data, stack trace, user info, endpoint). Admin portal at /admin/error-logs displays all errors with color-coded status badges and dropdown to update status. API endpoint PATCH /api/admin/error-logs/:id/status allows status updates. All errors default to 'to_do' status and can be marked as 'investigated' or 'fixed' during debugging workflow (October 17, 2025)
- **Article Generation Dialog**: Implemented AlertDialog that appears when users click "Generate Article" in Content Writer v2. The dialog informs users that article generation is in progress and they'll be notified when ready, with a "Go to Dashboard" button allowing navigation while generation continues in background. Includes proper error handling (dialog closes on failure with error message) and React best practices (mounted state ref prevents state updates on unmounted components). Backend notification system alerts users when article completes (October 17, 2025)
- **Content Writer v2 Feedback System**: Added thumbs up/down feedback buttons to both concept and subtopic cards in Content Writer v2. Users can provide instant feedback on AI-generated concepts and subtopics, which is stored in the content_feedback table and used by the RAG system to continuously improve future outputs. Feedback includes rating, input context, output data, and optional brand guideline association for personalized learning (October 17, 2025)
- **Auto-Assign Free Tier**: New users are now automatically assigned to the Free tier upon signup, granting immediate access to Content Writer v2 with 2 uses per day. Added `getFreeTier()` method to storage interface and updated signup endpoint to create tier subscription during user registration (October 17, 2025)
