...
} from "./utils/social-content-access";
import { PostgresCheckpointer } from "./langgraph/postgres-checkpointer";
...
const startSessionSchema = z.object({
  ...
  selectedPlatforms: z.array(
    z.enum(["Facebook", "Instagram", "TikTok", "X (Twitter)", "YouTube"])
  ),
  ...
  selectedTargetAudiences: z
    .union([z.literal("all"), z.literal("none"), z.array(z.number())])
    .optional(),
});

const approveWireframesSchema = z.object({
  approvedWireframeIds: z
    .array(z.string())
    .min(1, "At least one wireframe must be approved"),
...
export function registerSocialContentRoutes(app: Express): void {
      ...
      try {
        ...
        if (!accessControl.hasAccess) {
          return res
            ...
            .json({
              error:
                accessControl.message || "You don't have access to this tool",
            });
        }

        // Validate platform access
        const platformValidation = await validatePlatformAccess(
          userId,
          validatedData.selectedPlatforms
        );
        ...
        const initialState: SocialContentState = {
          ...
          selectedFormats: validatedData.selectedFormats as Record<
            Platform,
            string[]
          >,
          guidelineProfileId: validatedData.guidelineProfileId,
  ...
  // Get session state
  app.get(
    "/api/social-content/sessions/:sessionId",
    requireAuth,
    async (req: Request, res: Response) => {
      try {
