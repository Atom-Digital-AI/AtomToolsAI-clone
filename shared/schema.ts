...
export const products = pgTable("products", {
  ...
  availableSubfeatures: jsonb("available_subfeatures").default(
    sql`'{"bulk": true, "variations": true, "brand_guidelines": true}'::jsonb`
  ),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
...
export const userTierSubscriptions = pgTable(
...
export const crawlJobs = pgTable("crawl_jobs", {
  ...
  userId: varchar("user_id")
    .notNull()
    .references(() => users.id, { onDelete: "cascade" }),
  guidelineProfileId: varchar("guideline_profile_id").references(
    () => guidelineProfiles.id,
    { onDelete: "cascade" }
  ),
...
export const brandContextContent = pgTable("brand_context_content", {
  id: varchar("id")
    .primaryKey()
    .default(sql`gen_random_uuid()`),
  guidelineProfileId: varchar("guideline_profile_id")
    .notNull()
    .references(() => guidelineProfiles.id, { onDelete: "cascade" }),
...
export const brandEmbeddings = pgTable(
  "brand_embeddings",
  {
    ...
    userId: varchar("user_id")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }), // SECURITY: Direct user ownership for tenant isolation
    guidelineProfileId: varchar("guideline_profile_id")
      .notNull()
      .references(() => guidelineProfiles.id, { onDelete: "cascade" }),
    contextContentId: varchar("context_content_id").references(
      () => brandContextContent.id,
      { onDelete: "cascade" }
    ), // Null for general profile embeddings
...
export const pages = pgTable("pages", {
  id: varchar("id")
    .primaryKey()
    .default(sql`gen_random_uuid()`),
  crawlId: varchar("crawl_id")
    .notNull()
    .references(() => crawlJobs.id, { onDelete: "cascade" }),
  userId: varchar("user_id")
    .notNull()
    .references(() => users.id, { onDelete: "cascade" }), // For tenant isolation
...
export const completeProfileSchema = createInsertSchema(users)
  .pick({
    firstName: true,
    lastName: true,
    companyName: true,
...
export const insertGuidelineProfileSchema = createInsertSchema(
  guidelineProfiles
)
  .pick({
    name: true,
...
export const updateGuidelineProfileSchema = createInsertSchema(
  guidelineProfiles
)
  .pick({
    name: true,
...
export const updateCrawlJobSchema = createInsertSchema(crawlJobs)
  .pick({
    status: true,
    progress: true,
    results: true,
...
export const insertBrandContextContentSchema = createInsertSchema(
  brandContextContent
).pick({
  guidelineProfileId: true,
  url: true,
...
export const insertBrandEmbeddingSchema = createInsertSchema(
  brandEmbeddings
).pick({
...
export const insertPageReviewSchema = createInsertSchema(pageReviews)
  .pick({
    pageId: true,
    userId: true,
    classification: true,
...
export const insertExclusionRuleSchema = createInsertSchema(exclusionRules)
  .pick({
    userId: true,
    guidelineProfileId: true,
    ruleType: true,
...
export const contentWriterSessions = pgTable("content_writer_sessions", {
  ...
  selectedTargetAudiences: jsonb("selected_target_audiences").$type<
    "all" | "none" | number[]
  >(), // Target audience selection: "all", "none", or array of indices
...
export const contentWriterConcepts = pgTable("content_writer_concepts", {
  ...
  sessionId: varchar("session_id")
    .notNull()
    .references(() => contentWriterSessions.id, { onDelete: "cascade" }),
  title: text("title").notNull(),
...
export const contentWriterSubtopics = pgTable("content_writer_subtopics", {
  ...
  sessionId: varchar("session_id")
    .notNull()
    .references(() => contentWriterSessions.id, { onDelete: "cascade" }),
  parentConceptId: varchar("parent_concept_id")
    .notNull()
    .references(() => contentWriterConcepts.id),
  title: text("title").notNull(),
  summary: text("summary").notNull(),
...
export const contentWriterDrafts = pgTable("content_writer_drafts", {
  ...
  subtopicBriefs:
    jsonb("subtopic_briefs").$type<{ subtopicId: string; brief: string }[]>(), // Briefs for each subtopic
  subtopicContents:
    jsonb("subtopic_contents").$type<
      { subtopicId: string; content: string }[]
    >(), // Generated content per subtopic
...
export const langgraphThreads = pgTable(
  "langgraph_threads",
  {
    id: varchar("id")
      .primaryKey()
      .default(sql`gen_random_uuid()`), // thread_id
    userId: varchar("user_id")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    sessionId: varchar("session_id").references(
      () => contentWriterSessions.id,
      { onDelete: "cascade" }
    ), // Link to content writer session
  ...
  (table) => ({
    ...
    statusCreatedAtIdx: index("langgraph_threads_status_created_at_idx").on(
      table.status,
      table.createdAt
    ),
  })
...
export const langgraphCheckpoints = pgTable(
  "langgraph_checkpoints",
  {
    ...
    threadId: varchar("thread_id")
      .notNull()
      .references(() => langgraphThreads.id, { onDelete: "cascade" }),
  ...
  (table) => ({
    threadIdIdx: index("langgraph_checkpoints_thread_id_idx").on(
      table.threadId
    ),
    checkpointIdIdx: index("langgraph_checkpoints_checkpoint_id_idx").on(
      table.checkpointId
    ),
    threadCheckpointUnique: unique("thread_checkpoint_unique").on(
      table.threadId,
      table.checkpointId
    ),
  })
...
export type InsertLanggraphCheckpoint =
  typeof langgraphCheckpoints.$inferInsert;
...
export type InsertContentWriterSession =
  typeof contentWriterSessions.$inferInsert;
export type ContentWriterConcept = typeof contentWriterConcepts.$inferSelect;
export type InsertContentWriterConcept =
  typeof contentWriterConcepts.$inferInsert;
export type ContentWriterSubtopic = typeof contentWriterSubtopics.$inferSelect;
export type InsertContentWriterSubtopic =
  typeof contentWriterSubtopics.$inferInsert;
...
export const userNotificationPreferences = pgTable(
  "user_notification_preferences",
  {
    ...
    userId: varchar("user_id")
      ...
      .unique()
      .references(() => users.id, { onDelete: "cascade" }),
    emailOnArticleComplete: boolean("email_on_article_complete")
      .notNull()
      .default(true),
    emailOnSystemMessages: boolean("email_on_system_messages")
      .notNull()
      .default(true),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").defaultNow().notNull(),
...
export const adSpecs = pgTable(
  ...
  (table) => ({
    platformFormatIdx: index("ad_specs_platform_format_idx").on(
      table.platform,
      table.format
    ),
    platformFormatVersionUnique: unique("platform_format_version_unique").on(
      table.platform,
      table.format,
      table.version
    ),
...
export const socialContentSessions = pgTable("social_content_sessions", {
  ...
  selectedFormats: jsonb("selected_formats")
    ...
    .notNull(), // { 'Facebook': ['Feed Image Ad', ...], ... }
  scrapedUrlData:
    jsonb("scraped_url_data").$type<
      Array<{ url: string; summary: string; keyPoints: string[] }>
    >(), // Session URL scraping results
...
export type InsertSocialContentSession =
  typeof socialContentSessions.$inferInsert;
...
export const socialContentWireframes = pgTable(
  "social_content_wireframes",
  {
    id: varchar("id")
      .primaryKey()
      .default(sql`gen_random_uuid()`),
    sessionId: varchar("session_id")
      .notNull()
      .references(() => socialContentSessions.id, { onDelete: "cascade" }),
  ...
  (table) => ({
    sessionPlatformFormatIdx: index(
      "wireframes_session_platform_format_idx"
    ).on(table.sessionId, table.platform, table.format),
  })
);

export type SocialContentWireframe =
  typeof socialContentWireframes.$inferSelect;
export type InsertSocialContentWireframe =
  typeof socialContentWireframes.$inferInsert;
...
export const qcUserDecisions = pgTable(
  "qc_user_decisions",
  {
    id: varchar("id")
      .primaryKey()
  ...
  (table) => ({
    ...
    guidelineProfileIdIdx: index(
      "qc_user_decisions_guideline_profile_id_idx"
    ).on(table.guidelineProfileId),
  })
);

// QC Configurations - User/brand-specific QC preferences
export const qcConfigurations = pgTable(
  "qc_configurations",
  {
    id: varchar("id")
      .primaryKey()
    ...
    enabledAgents: jsonb("enabled_agents")
      .$type<QCAgentType[]>()
      .default(
        sql`'["proofreader", "brand_guardian", "fact_checker", "regulatory"]'::jsonb`
      ),
    ...
    conflictResolutionStrategy: varchar("conflict_resolution_strategy", {
      length: 50,
    }).default("human_review"), // 'human_review', 'learned_preferences', 'priority_based'

    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").defaultNow().notNull(),
  },
  (table) => ({
    userToolUnique: unique("user_tool_qc_config_unique").on(
      table.userId,
      table.guidelineProfileId,
      table.toolType
    ),
...
export type InsertUserTierSubscription =
  typeof userTierSubscriptions.$inferInsert;

export type GuidelineProfile = Omit<
  typeof guidelineProfiles.$inferSelect,
  "content"
> & {
  content: GuidelineContent;
};
export type InsertGuidelineProfile = z.infer<
  typeof insertGuidelineProfileSchema
>;
export type UpdateGuidelineProfile = z.infer<
  typeof updateGuidelineProfileSchema
>;
...
export type InsertBrandContextContent = z.infer<
  typeof insertBrandContextContentSchema
>;
...
export const packageProductsRelations = relations(
  packageProducts,
  ({ one }) => ({
...
export const cmsNavigationRelations = relations(
  cmsNavigation,
  ({ one, many }) => ({
...
export const guidelineProfilesRelations = relations(
  guidelineProfiles,
  ({ one, many }) => ({
    user: one(users, {
...
export const brandContextContentRelations = relations(
  brandContextContent,
  ({ one }) => ({
...
export const userTierSubscriptionsRelations = relations(
