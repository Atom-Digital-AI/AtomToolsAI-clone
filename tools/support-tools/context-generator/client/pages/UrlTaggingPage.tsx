...
interface CrawledUrl {
  url: string;
  title: string;
  autoCategory?: PageType; // Auto-categorization from server
}
...
type PageType = "untagged" | "about" | "blog" | "product" | "legals" | "company" | "knowledge";
...
export default function UrlTaggingPage({
  ...
  const [tags, setTags] = useState<Record<string, PageType>>(
    Object.fromEntries(crawledUrls.map(u => [u.url, u.autoCategory || "untagged" as PageType]))
  );
  ...
  const handleSubmit = () => {
    ...
    const legals = Object.keys(tags).filter(url => tags[url] === "legals");
    const companyPages = Object.keys(tags).filter(url => tags[url] === "company");
    const knowledgeBase = Object.keys(tags).filter(url => tags[url] === "knowledge");
    ...
    onSubmit({ about, products, blogs, legals, companyPages, knowledgeBase });
  };
  ...
  const stats = {
    ...
    legals: Object.values(tags).filter(t => t === "legals").length,
    company: Object.values(tags).filter(t => t === "company").length,
    knowledge: Object.values(tags).filter(t => t === "knowledge").length,
  };
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6">
      <div className="max-w-[90%] mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="p-4 rounded-lg border-2 bg-gray-900 border-gray-700">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-400">Legals</p>
                <p className="text-2xl font-bold">{stats.legals}</p>
          ...
          <div className="p-4 rounded-lg border-2 bg-gray-900 border-gray-700">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-400">Company Pages</p>
                    ...
                    <td className="px-4 py-3">
                      ...
                      >
                        ...
                        <SelectContent className="bg-gray-800 border-gray-700">
                          ...
                          <SelectItem value="legals" className="text-orange-400">
                            Legals
                          </SelectItem>
                          <SelectItem value="company" className="text-yellow-400">
                            Company Pages
                          </SelectItem>
                          <SelectItem value="knowledge" className="text-cyan-400">
                            Knowledge Base
                          </SelectItem>
                        </SelectContent>
                      </Select>
                      {crawledUrl.autoCategory && crawledUrl.autoCategory !== tags[crawledUrl.url] && (
                        <p className="text-xs text-gray-500 mt-1">
                          Auto: {crawledUrl.autoCategory}
                        </p>
                      )}
                    </td>
                  </tr>
