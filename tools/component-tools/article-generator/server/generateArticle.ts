...
} from "../../../../server/utils/language-helpers";
...
export async function generateArticle(
  ...
  try {
    ...
    const chosenConcept = concepts.find((c: any) => c.id === selectedConceptId);
    if (!chosenConcept) {
      throw new Error("Selected concept not found");
    }

    const selectedSubtopics = subtopics.filter(
      (s: any) => s.id && selectedSubtopicIds.includes(s.id)
    );
    ...
    const mainBriefPrompt = await loadPrompt(
      toolPath,
      "../outline-builder/main-brief",
      {
    ...
    for (const subtopic of selectedSubtopics) {
      const subtopicBriefPrompt = await loadPrompt(toolPath, "subtopic-brief", {
    ...
    for (const subtopic of selectedSubtopics) {
      ...
      const contentPrompt = await loadPrompt(toolPath, "subtopic-content", {
        ...
        targetWordCount: Math.floor(
          (targetLength || 1000) / selectedSubtopics.length
        ),
      });
    ...
    const topTailPrompt = await loadPrompt(toolPath, "intro-conclusion", {
      conceptTitle: chosenConcept.title,
      mainBrief,
      targetAudienceContext,
      brandContext: brandContext || "",
    ...
    for (const subtopic of selectedSubtopics) {
      const content =
        subtopicContents.find((sc) => sc.subtopicId === subtopic.id)?.content ||
        "";
