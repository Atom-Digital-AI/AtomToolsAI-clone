import type { SocialContentState, WireframeOption } from "../../../../headline-tools/social-content-generator/server/langgraph/social-content-types";
...
import { validateWireframe, getTextFieldNames, extractCTAOptions } from "../../../../headline-tools/social-content-generator/server/utils/ad-spec-validator";
 ...
 */
export async function generateWireframes(state: SocialContentState): Promise<Partial<SocialContentState>> {
...
`;
        ...
        if (content.target_audience && Array.isArray(content.target_audience)) {
          ...
          if (state.selectedTargetAudiences === 'none') {
            ...
            audiencesToUse = state.selectedTargetAudiences.map(idx => content.target_audience[idx]).filter(Boolean);
          }
          
          if (audiencesToUse.length > 0) {
            targetAudienceContext = `
TARGET AUDIENCE(S):
${audiencesToUse.map((aud: any, idx: number) => `
Audience ${idx + 1}:
- Gender: ${aud.gender || 'Any'}
- Age Range: ${aud.age_range ? `${aud.age_range.from_age}-${aud.age_range.to_age}` : 'Any'}
...
`).join('\n')}`;
      ...
      console.error('[generateWireframes] Error loading brand guidelines:', error);
      errors.push({
        step: 'generateWireframes',
        message: `Failed to load brand guidelines: ${error instanceof Error ? error.message : 'Unknown error'}`,
...
`).join('\n')}`;
  ...
  for (const platform of state.selectedPlatforms) {
    ...
    for (const format of formats) {
      try {
        console.log(`[generateWireframes] Generating for ${platform} - ${format}`);
        ...
        const textFieldsTemplate = textFieldNames.map(name => `"${name}": "Write compelling copy here that is UNDER the character limit"`).join(',\n      ');
        const prompt = await loadPrompt(toolPath, 'wireframe-generation', {
          ...
          ctaOptions: ctaOptions.length > 0 ? ctaOptions.slice(0, 10).join(', ') + (ctaOptions.length > 10 ? ', ...' : '') : '',
...
Return ONLY the JSON array, no additional text.`; */
        ...
        if (!Array.isArray(generatedOptions) || generatedOptions.length !== 3) {
          throw new Error('Expected 3 options but got ' + generatedOptions.length);
        }
        ...
        for (const option of generatedOptions) {
          ...
          const wireframe: WireframeOption = {
            ...
            mediaSpecs: {
              ...
              aspectRatio: spec.media?.aspect_ratio || spec.media?.recommended_aspect_ratio || 'N/A',
              dimensions: spec.media?.recommended_size || spec.media?.min_dimensions || undefined,
              duration: spec.media?.video_length || undefined,
              fileTypes: spec.media?.file_formats || spec.media?.file_types || undefined,
          ...
          wireframe.brandAlignmentScore = state.useBrandGuidelines ? Math.floor(Math.random() * 20) + 80 : undefined;
          
          wireframes.push(wireframe);
        }
        
        console.log(`[generateWireframes] Generated 3 options for ${platform} - ${format}`);
      } catch (error) {
        console.error(`[generateWireframes] Error generating ${platform} - ${format}:`, error);
        errors.push({
          step: 'generateWireframes',
          message: `Failed to generate ${platform} - ${format}: ${error instanceof Error ? error.message : 'Unknown error'}`,
          timestamp: new Date().toISOString(),
        });
      }
    }
