...
export default function ContentHistory() {
  ...
  const filteredContents = allContent.filter((content: any) => {
    const matchesSearch =
      ...
      JSON.stringify(content.inputData)
        .toLowerCase()
        .includes(searchTerm.toLowerCase());

    const matchesFilter =
      toolFilter === "all" || content.toolType === toolFilter;
  ...
  const downloadContent = (content: any) => {
    ...
    if (content.toolType === "content-writer-v2") {
      ...
      fileName = `article-${content.title
        .replace(/[^a-z0-9]/gi, "_")
        .toLowerCase()}.md`;
      ...
      fileName = `google-ads-${content.title
        .replace(/[^a-z0-9]/gi, "_")
        .toLowerCase()}.csv`;
      mimeType = "text/csv";
    } else if (content.toolType === "seo-meta") {
      ...
      fileContent = csvRows.join("\n");
      fileName = `seo-meta-${content.title
        .replace(/[^a-z0-9]/gi, "_")
        .toLowerCase()}.csv`;
      mimeType = "text/csv";
    }
    ...
    toast({
      ...
      description: `Your content is being downloaded as ${fileName
        .split(".")
        .pop()
        ?.toUpperCase()}.`,
    });
      ...
      return (
        <div className="space-y-4">
          <div>
            ...
            <div className="bg-gray-800 p-4 rounded text-sm text-gray-300 max-h-96 overflow-y-auto prose prose-invert max-w-none whitespace-pre-wrap">
              {data.finalArticle || ""}
            </div>
          </div>
        ...
        <div className="mb-6 flex flex-col sm:flex-row gap-4">
          ...
          <Select value={toolFilter} onValueChange={setToolFilter}>
            ...
            <SelectContent>
              ...
              <SelectItem value="content-writer-v2">
                Content Writer v2
              </SelectItem>
            </SelectContent>
          </Select>
                  ...
                  <div className="text-gray-300 text-sm">
                    {content.toolType === "google-ads" && (
                      <div>
                        <p>
                          ...
                          {(content.inputData as any).targetKeywords || "N/A"}
                        </p>
                        <p>
                          <strong>Brand:</strong>{" "}
                          {(content.inputData as any).brandName || "N/A"}
                        </p>
                      </div>
                    )}
                    {content.toolType === "seo-meta" && (
                      <div>
                        ...
                        <p>
                          <strong>Type:</strong>{" "}
                          {(content.inputData as any).contentType || "N/A"}
                        </p>
                      </div>
                    )}
                    {content.toolType === "content-writer-v2" && (
                      <div>
                        <p>
                          <strong>Topic:</strong>{" "}
                          {(content.inputData as any).topic || "N/A"}
                        </p>
                        <p>
                          <strong>Word Count:</strong>{" "}
                          {(content.inputData as any).wordCount || "N/A"}
                        </p>
                      </div>
                    )}
                  </div>
          ...
          <DialogContent className="max-w-4xl max-h-[80vh] bg-gray-900 border-gray-700 overflow-hidden">
            <DialogHeader>
              ...
              <DialogDescription className="text-gray-400">
                Generated on{" "}
                {selectedContent && formatDate(selectedContent.createdAt)}
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="flex justify-end gap-2 pt-4 border-t border-gray-700">
              <Button
                variant="outline"
                onClick={() =>
                  selectedContent && copyToClipboard(selectedContent)
                }
              ...
              <Button
                variant="outline"
                onClick={() =>
                  selectedContent && downloadContent(selectedContent)
                }
                className="bg-gray-800 border-gray-600 text-gray-300 hover:bg-gray-700 hover:text-white"
              >
                <Download className="mr-2 h-4 w-4" />
